...
cbuffer SpotShadowOverlayParams : register(b0)
{
    row_major float4x4 invViewProjection;
    row_major float4x4 spotShadowVP;
...
float CalculateSpotShadow(float3 worldPos)
{
    float4 lightClipPos = mul(float4(worldPos, 1.0), spotShadowVP);
...
float4 main(PSInput input) : SV_TARGET
{
    ...
    // Outside cone = blue tint
    float cosAngle = dot(lightDirection, normalize(lightToFrag));
    if (cosAngle < outerCos)
        return float4(0.0, 0.0, 0.4, overlayAlpha);

    // Shadow test
    float shadow = CalculateSpotShadow(worldPos.xyz);
